...
> **链：** Sui  
> **状态：** 草案  
> **背景阅读：** [docs/qa.zh.md](./docs/qa.zh.md) · **数学规范：** [math-spec/SPEC.zh.md](./math-spec/SPEC.zh.md) · **预言机操作手册：** [docs/oracle-playbook.zh.md](./docs/oracle-playbook.zh.md) · **知识付费生态：** [SuiProphet_Network.zh.md](./SuiProphet_Network.zh.md)
...
- Vol Crush、IV、Vol Smile——详见 [docs/qa.zh.md](./docs/qa.zh.md)；**Sui Indexer 独立**计算与展示
...
> 产品逻辑见 [docs/qa.zh.md](./docs/qa.zh.md)（流动性注入、Opening Auction）。
...
> 完整案例与公式：[docs/qa.zh.md](./docs/qa.zh.md)（皇马 vs 巴萨、选举、CPI、盈亏平衡测算）。
...
> 调研 [docs/qa.zh.md](./docs/qa.zh.md)「顶级工程防守」。
...
> 调研 [docs/qa.zh.md](./docs/qa.zh.md)「LP 的收益呢？特别是 LP 加入时机不一样」。
...
**加入时机：** $T_0$ 开盘拓荒、$T_1$ 中场溢价扩容、$T_2$ 末期禁入（封盘前 N% 拒绝 `deposit_liquidity`）；详见 [docs/qa.zh.md](./docs/qa.zh.md)。
...
> **决策（2026-06-08）：** 主网前不上 Tier 2 联合 PDF 模型；见 [docs/tier2-decision.zh.md](./docs/tier2-decision.zh.md)。
...
> 排行与 Prophet Score **不以 Indexer 为真相源**；MVP 前端经 RPC + 事件扫描直读链上动态字段即可。详见 [docs/phase4-services.zh.md](./docs/phase4-services.zh.md)。
...
7. **动态费率与虚拟流动性：** 检测窗口、衰减曲线（见 docs/qa.zh.md §LP 防守） -> **已实现**：`services/lp-guard-keeper/` 链下 Keeper 轮询 `MarketPool`，按漂移/偏度/成交量 EMA 计算风险分，调用 `pool::set_lp_guard_params` 动态拉高 `fee_multiplier_bps` 与虚拟 σ/浓度；平静时 `LP_GUARD_DECAY_FACTOR`（默认 0.85）平滑衰减。链上计费见 `lp_guard.move`；运维见 `docs/phase2-playbook.zh.md` §1.3。
...
| 操作手册 | [docs/oracle-playbook.zh.md](./docs/oracle-playbook.zh.md) |
...
- [uma1.zh.md](./uma1.zh.md) — 仲裁状态机、Slasher Math、五态枚举  
- [uma2.zh.md](./uma2.zh.md) — 预言机术语标准化、Optimistic Engine、仲裁 Outbound/Inbound 规范  

---

## 11. SuiProphet Network（知识付费模块）

> 完整愿景文档：[SuiProphet_Network.zh.md](./SuiProphet_Network.zh.md)  
...
- [SuiProphet_Network.zh.md](./SuiProphet_Network.zh.md) — 原始产品愿景  
