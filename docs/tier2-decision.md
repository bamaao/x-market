...
> **状态：** 已决议  
> **关联：** [PRD.md](../PRD.md) §3.6 · [math-spec/SPEC.md](../math-spec/SPEC.md) §9 · [docs/qa.md](./qa.md) §Tier 1/2 · [phase3-playbook.md](./phase3-playbook.md) §4 · [slash-and-attestation.md](./slash-and-attestation.md) · [deferred-features.md](./deferred-features.md)
...
常见误解：Variance Swap 等 Phase 3 能力已上线，但它们不触发 Tier 2；只有 **联合高维 PDF** 才需要 Tier 2 架构（见 [qa.md](./qa.md) §Tier 2）。
...
| 得票率 [0, 1] | Dirichlet 近似 Beta（[SPEC.md](../math-spec/SPEC.md) §7） | ✅ 规格已定 |
| 「主队赢 AND 进球 > 2.5」 | **独立双池 + Indexer 乘法**（[SPEC.md](../math-spec/SPEC.md) §9） | ✅ MVP 路径 |
...
Tier 1 毫秒原子、零外部依赖是核心工程优势；Tier 2 本质为算力换可验证性，不应替代主路径。详见 [qa.md](./qa.md) §Tier 1/2 对比。
...
与 Tier 2 无关、但 PRD 有规划的 **Tier 1 扩展 / Oracle 治理** 缺口，见 [deferred-features.md](./deferred-features.md)。Normal 竞价与 UMA DVM 适配器已实现；主网前仍不阻断项：Beta CDF。
...
- **Attestation + 挑战约束**：性能满足热路径登记；信任依赖验证委员会 + slash（与 Macro Oracle 同类模型）；详见 [slash-and-attestation.md](./slash-and-attestation.md)
...
参考阈值（[mainnet-infra-priority.md](./mainnet-infra-priority.md)）：活跃市场 > 50 或 Prophet 预测 > 200 之前，上述条件通常不会成为真痛点。
...
| 2026-06-08 | v1.1 | 增加 §4 延期项索引，链接 [deferred-features.md](./deferred-features.md) |
